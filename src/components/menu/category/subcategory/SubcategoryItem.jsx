  ...
  return (
    ...
    >
      ...
      <div className={`gap-2 ${isHovered ? 'ml-2' : ''} flex`}>
        ...
        {
          isHovered && <div className={`gap-2 flex transition-all duration-200`}>
            <button
              className={`text-sm text-white hover:text-yellow-300 transition-colors cursor-pointer`}
              onClick={() => handleEditSubcategoryClick(category.id, subcategory)}
            >
              <ICONS.EDIT />
            </button>
            <button
              className={`text-sm text-white hover:text-red-300 transition-colors cursor-pointer`}
              onClick={() => handleDeleteSubcategoryClick(category.id, subcategory.id)}
            >
              <ICONS.DELETE />
            </button>
          </div>
